...
| MEMORY.md.draft | `memory/MEMORY.md.draft` | Candidate core memory draft. On startup, first `read_file` `memory/MEMORY.md` (current SQLite-derived ground truth), then generate the draft to overwrite this file. |
| ~~MEMORY.md~~ | ~~`memory/MEMORY.md`~~ | **Do not write.** MEMORY.md is auto-generated by the memory system from SQLite. Your draft is only a suggestion. |
...
- "Project targets indie developers, ~10K stars" → MEMORY.md (via draft)
- "Reverse proxy on port 8080 with user deploy" → MEMORY.md (via draft, infrastructure overview)
...
1. **On startup**: `read_file` → `memory/MEMORY.md` to see the current SQLite-derived ground truth.
2. **Analyze**: Entries in `history.jsonl` after the cursor (unprocessed by previous Dream runs). Distill new facts.
3. **Generate**: Produce `memory/MEMORY.md.draft` with the same format as MEMORY.md:
   ```markdown
   # 核心记忆
   ...
   - ...
   ```
4. **Conflict detection**: If a new fact contradicts existing MEMORY.md content, add a note at the top of the draft:
   ```markdown
   # Draft notes — YYYY-MM-DD
   潜在冲突：...
   ```
5. **Write**: `write_file` → `memory/MEMORY.md.draft` (overwrite previous draft). **Do NOT write MEMORY.md.**

## SKILL.md creation (unchanged)

- Create when: reusable workflow appears ≥ 2 times with clear, actionable steps
- Do NOT overwrite existing skills — merge delta into the existing skill on conflict
...
- [skip]: audit-only or non-SNIP content. Do not write it to SOUL.md, USER.md, MEMORY.md.draft, or SKILL.md.
...
- After migrating content to a skill, delete it from the source file (MEMORY.md.draft or USER.md) to maintain MECE
